...
def model(Y, t, pv=210, delta_v=5, kl=0.4, beta=5e-7, gt=0.8, beta_prim=3e-8, rho=2.6, fi=0.33, delta_i=2,
    ...
    viral_load, target_cells, infected_cells, ifn, virus_resistance, naive_CD8, effector, naive_B, plasma_cells, short_antibodies, long_antibodies, memory_CD8, memory_effector = Y(t)

    viral_load_tau_c, naive_CD8_tau_c = Y(t - tau_c)[0], Y(t - tau_c)[5]
    viral_load_tau_b, naive_B_tau_b = Y(t - tau_b)[0], Y(t - tau_c)[7]
    viral_load_tau_cm, memory_CD8_tau_cm = Y(t - tau_b)[0], Y(t - tau_c)[7]
